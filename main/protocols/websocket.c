 ...
 * Using esp_websocket_client component from ESP-IDF registry
 */
                ...
                if (root) {
                    ...
                    if (type && cJSON_IsString(type)) {
                        if (strcmp(type->valuestring, "audio") == 0) {
                            ...
                            ESP_LOGI(TAG, "Incoming audio data");
...
void websocket_send_audio_frame(audio_frame_t *frame)
{
    ...
    // Send raw audio data as binary frame
    esp_err_t err = esp_websocket_client_send_bin(g_client, 
...
bool websocket_has_incoming_audio(void)
{
    ...
    g_has_incoming_audio = false; // Reset flag
    return has_audio;
}
