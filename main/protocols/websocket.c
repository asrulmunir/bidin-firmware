 ...
 * Using esp_http_client with WebSocket URL (ws://)
 * ESP-IDF v5.5 automatically uses WebSocket transport for ws:// URLs
 */
...
void websocket_init(void)
{
    ...
    // Configure HTTP client (WebSocket URL will use ws:// transport)
...
void websocket_connect(void)
{
    ...
    // Open connection (WebSocket handshake happens automatically for ws:// URLs)
...
void websocket_send_hello(void)
{
    ...
    // Set HTTP method to POST for sending data
    esp_http_client_set_method(g_client, HTTP_METHOD_POST);
    esp_http_client_set_header(g_client, "Content-Type", "application/json");
    
    esp_err_t err = esp_http_client_write(g_client, json_str, strlen(json_str));
    if (err != ESP_OK) {
...
void websocket_send_audio_start(void)
{
    ...
    esp_err_t err = esp_http_client_write(g_client, json_str, strlen(json_str));
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send audio start: %s", esp_err_to_name(err));
    }
...
void websocket_send_audio_frame(audio_frame_t *frame)
{
    ...
    esp_err_t err = esp_http_client_write(g_client, 
                                           (const char *)frame->samples, 
                                           frame->count * sizeof(int16_t));
    if (err != ESP_OK) {
        ESP_LOGV(TAG, "Failed to send audio frame: %s", esp_err_to_name(err));
    }
}
...
void websocket_send_audio_end(void)
{
    ...
    esp_err_t err = esp_http_client_write(g_client, json_str, strlen(json_str));
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send audio end: %s", esp_err_to_name(err));
    }
...
{
    // TODO: Implement proper receive buffer checking
...
bool websocket_read_audio_frame(audio_frame_t *frame)
{
    ...
    // TODO: Implement proper audio receive
    return false;
}
